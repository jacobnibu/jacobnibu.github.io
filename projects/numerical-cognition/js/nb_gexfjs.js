        ...
        success: function(data) {
            ...
            $(_nodes).each( function() {
                var _n = $(this),
					...
					_keywords = _n.attr("keywords") || _id,
                    _d = {
                        id: _id,
                        label: _label,
						keywords: _keywords
				...
				GexfJS.graph.nodeIndexByKeywords.push(_keywords.toLowerCase());
...
function hoverAC2() {
    $("#autocomplete2 li").removeClass("hover");
    $("#liac_"+GexfJS.autoCompletePosition).addClass("hover");
    GexfJS.params.activeNode = GexfJS.graph.nodeIndexByLabel.indexOf( $("#liac_"+GexfJS.autoCompletePosition).text().toLowerCase() );
}
...
/* function updateAutoComplete2(_sender) {
    var _val = $(_sender).val().toLowerCase();
    var _ac = $("#autocomplete2");
    if (_val != GexfJS.dernierAC || _ac.html() == "") {
        GexfJS.dernierAC = _val;
        var _strAC = "<div><h4>" + strLang("nodes") + "</h4><ul>";
        var _n = 0;
        for (var i in GexfJS.graph.nodeIndexByKeywords) {
            var _l = GexfJS.graph.nodeIndexByKeywords[i];
            if (_l.search(_val) != -1) {
                _strAC += '<li id="liac_' + _n + '" onmouseover="changePosAC2(' + _n + ')"><a href="#" onclick="displayNode(\'' + i + '\', true); return false;"><span>' + GexfJS.graph.nodeList[i].label + '</span></a>';
                _n++;
            }
            if (_n >= 20) {
                break;
...
$(document).ready(function() {
	...
	/* $("#searchinput2")
        ...
        .keyup(function(evt) {
            updateAutoComplete2(this);
        }).keydown(function(evt){
            var _l = $("#autocomplete2 li").length;
            switch (evt.keyCode) {
                case 40 :
                    if (GexfJS.autoCompletePosition < _l - 1) {
                        GexfJS.autoCompletePosition++;
                ...
                break;
                case 27 :
                    $("#autocomplete2").slideUp();
                break;
                case 13 :
                    if ($("#autocomplete2").is(":visible")) {
                        var _liac = $("#liac_"+GexfJS.autoCompletePosition);
                        if (_liac.length) {
                            $(this).val(_liac.find("span").text());
                        }
	...
	$("#recherche2").submit(function() {
        if (GexfJS.graph) {
            displayNode( GexfJS.graph.nodeIndexByKeywords.indexOf($("#searchinput2").val().toLowerCase()), true);
        }
        return false;
    }); */
    $("#carte")
	...
	$("#autocomplete2").css({
        top: ( $("#searchinput2").offset().top + $("#searchinput2").outerHeight() ) + "px",
        left: $("#searchinput2").offset().left + "px"
    });
